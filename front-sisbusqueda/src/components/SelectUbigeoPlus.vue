...
onBeforeMount(async () => {
  ...
  if (banderaModelValue) {
    ...
    model_depa.value = stringDepartamentos.find(v => v.cod_dep === codigo_array[0]);
    model_prov.value = stringProvincias.find(v => v.cod_prov === codigo_array[1]);
    model_dist.value = stringDistritos.find(v => v.cod_dist === codigo_array[2]);
    console.log(model_dist.value);
...
function filterDepartamentos(val, update) {
  update(() => {
    if (val === '') {
        optionsDepartamentos.value = stringDepartamentos;
    } else {
...
function filterProvincias(val, update) {
  update(() => {
    if (val === '') {
      optionsProvincias.value = stringProvincias;
    } else {
...
function filterDistritos(val, update) {
    update(() => {
      if (val === '') {
        optionsDistritos.value = stringDistritos;
      } else {
